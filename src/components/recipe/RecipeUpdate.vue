<template>
  <div>
    <v-alert type="error" v-model="alert" dismissable>Update of recipe has failed.</v-alert>
    <v-form>
      ...
      <v-row v-for="(ingredient, counterr) in recipe.ingredients" :key="counterr + 1">
        <v-col sm="6">
      ...
      <v-btn @click="dialog = true" class="mr-4">Update</v-btn>
    </v-form>
      ...
      <v-dialog v-model="dialog" width="250">
        <v-card>
          ...
          <v-card-actions>
            ...
            <v-btn color="primary" text @click="updateRecipe">Yes</v-btn>
